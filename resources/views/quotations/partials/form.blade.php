...
<div class="mb-3">
    ...
    <select name="communication_method" class="form-control" required>
        ...
        @foreach (['Email', 'Phone', 'Messenger'] as $method)
            <option value="{{ $method }}"
                {{ old('communication_method', $isEdit ? $quotation->communication_method : '') == $method ? 'selected' : '' }}>
                {{ $method }}
            </option>
        @endforeach
    </select>
...
<div class="mb-3">
    ...
    <select name="budget_range" class="form-control" required>
        ...
        <option value="under_50k"
            {{ old('budget_range', $isEdit ? $quotation->budget_range : '') == 'under_50k' ? 'selected' : '' }}>
            Under ₱50,000</option>
        <option value="50k_100k"
            {{ old('budget_range', $isEdit ? $quotation->budget_range : '') == '50k_100k' ? 'selected' : '' }}>
            ₱50,000 - ₱100,000</option>
        <option value="100k_200k"
            {{ old('budget_range', $isEdit ? $quotation->budget_range : '') == '100k_200k' ? 'selected' : '' }}>
            ₱100,000 - ₱200,000</option>
        <option value="200k_plus"
            {{ old('budget_range', $isEdit ? $quotation->budget_range : '') == '200k_plus' ? 'selected' : '' }}>
...
<div class="mb-3">
    ...
    <div class="form-check">
        <input type="checkbox" name="package_deal" value="1" class="form-check-input"
            {{ old('package_deal', $isEdit ? $quotation->package_deal : false) ? 'checked' : '' }}>
        <label class="form-check-label">Package Deal</label>
    </div>

    <div class="form-check">
        <input type="checkbox" name="catering" value="1" class="form-check-input"
            {{ old('catering', $isEdit ? $quotation->catering : false) ? 'checked' : '' }}>
        <label class="form-check-label">Catering</label>
    </div>

    <div class="form-check">
        <input type="checkbox" name="hair_makeup" value="1" class="form-check-input"
            {{ old('hair_makeup', $isEdit ? $quotation->hair_makeup : false) ? 'checked' : '' }}>
        <label class="form-check-label">Hair & Makeup</label>
    </div>

    <div class="form-check">
        <input type="checkbox" name="live_band" value="1" class="form-check-input"
            {{ old('live_band', $isEdit ? $quotation->live_band : false) ? 'checked' : '' }}>
