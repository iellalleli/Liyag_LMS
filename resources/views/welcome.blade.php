...
<div class="container text-center py-5">
    ...
    @auth
        @if(auth()->user()->hasRole('admin'))
            <a href="{{ route('admin.dashboard') }}" class="btn btn-primary btn-lg mx-2">Go to Admin Dashboard</a>
        @elseif(auth()->user()->hasRole('sales_rep'))
            <a href="{{ route('sales.dashboard') }}" class="btn btn-primary btn-lg mx-2">Go to Sales Rep Dashboard</a>
        @elseif(auth()->user()->hasRole('client'))
            <a href="{{ route('client.dashboard') }}" class="btn btn-primary btn-lg mx-2">Go to Client Dashboard</a>
        @endif
    @else
